...
build-all-ci:
	GOOS=linux GOARCH=amd64 go build $(ARGS) -o bin/taskName-Linux-x86_64 -ldflags "-s -w"
	GOOS=linux GOARCH=arm64 go build $(ARGS) -o bin/taskName-Linux-ARM64 -ldflags "-s -w"
	GOOS=darwin GOARCH=amd64 go build $(ARGS) -o bin/taskName-Darwin-x86_64 -ldflags "-s -w"
	GOOS=darwin GOARCH=arm64 go build $(ARGS) -o bin/taskName-Darwin-ARM64 -ldflags "-s -w"
	GOOS=windows GOARCH=amd64 go build $(ARGS) -o bin/taskName-Windows-x86_64 -ldflags "-s -w"
